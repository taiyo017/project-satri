...
<section class="py-16 lg:py-24 px-6 sm:px-8 lg:px-12 bg-white dark:bg-gray-950">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="text-center max-w-3xl mx-auto mb-12">
            ...
            @if ($content)
                <div class="text-[16px] leading-[26px] mt-0 mb-4 text-gray-600 dark:text-gray-400">
                    {!! $content !!}
                </div>
            @endif
        </div>
        ...
        @if ($layout === 'grid')
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach ($features as $index => $item)
                    <div
                        class="group relative p-6 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-[#1363C6]/30 dark:hover:border-[#1363C6]/40 shadow-sm hover:shadow-lg hover:shadow-[#1363C6]/5 dark:hover:shadow-[#1363C6]/10 hover:-translate-y-0.5 transition-all duration-300 text-center">

                        {{-- ICON - Centered & Compact --}}
                        @if (!empty($item['icon']))
                            <div
                                class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-[#1363C6]/10 dark:bg-[#1363C6]/20 text-[#1363C6] dark:text-[#4a8dd8] mb-4 group-hover:scale-105 transition-transform duration-300">
                        ...
                        {{-- TITLE --}}
                        @if (!empty($item['title']))
                            <h3 class="text-[18px] leading-[24px] mt-3 mb-3 font-bold text-gray-900 dark:text-white">
                                {{ $item['title'] }}
                            </h3>
                        @endif
