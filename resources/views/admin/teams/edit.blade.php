    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('team-members.update', $teamMember->id) }}" method="POST"
                enctype="multipart/form-data" id="teamMemberForm">
                @csrf
                    ...
                    <div class="p-8">
                            ...
                            <div class="flex items-start gap-6">
                                ...
                                <div class="relative group">
                                    <div id="photoPreview"
                                        ...
                                        {{-- Default Icon --}}
                                        <div id="defaultIcon"
                                            class="{{ $teamMember->photo ? 'hidden' : '' }} text-center">
                                        ...
                                        {{-- Preview Image --}}
                                        <img id="previewImg" src="{{ asset('storage/' . $teamMember->photo) }}"
                                            alt="Preview"
                                            class="{{ $teamMember->photo ? '' : 'hidden' }} w-full h-full object-cover">
                                    </div>

                                    {{-- Remove Button --}}
                                    <button type="button" id="removePhotoBtn" onclick="removePhoto()"
                                        class="{{ $teamMember->photo ? '' : 'hidden' }} absolute -top-2 -right-2 w-8 h-8 bg-red-500 hover:bg-red-600 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="designation" placeholder="Chief Executive Officer"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="bio" rows="4"
                                class="w-full px-4 py-3 border-2 border-gray-300 dark:border-gray-700 rounded-xl  focus:border-[#1363C6] focus:ring-4 focus:ring-[#1363C6]/10 dark:bg-gray-800
                                dark:text-white transition-all duration-300 resize-none"
                                placeholder="Write a brief bio about the team member...">{{ old('bio', $teamMember->bio) }}</textarea>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="message" rows="4"
                                ...
                                placeholder="Write a message from the team member...">{{ old('message', $teamMember->message) }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="number" name="order_index"
                                value="{{ old('order_index', $teamMember->order_index) }}" min="0"
                                class="w-full px-4 py-3 border-2 border-gray-300 dark:border-gray-700 rounded-xl 
                                focus:border-[#1363C6] focus:ring-4 focus:ring-[#1363C6]/10 
                                dark:bg-gray-800 dark:text-white transition-all duration-300">
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Update member's social profiles
                                    </p>
                                </div>
                            </div>
                    ...
                    <div class="p-8">
                        <div id="socialLinksContainer" class="space-y-4">
                            @if ($teamMember->social_links && count($teamMember->social_links) > 0)
                                @foreach ($teamMember->social_links as $i => $link)
                                    <script>
                                        document.addEventListener('DOMContentLoaded', function() {
                                            addSocialLink();

                                            // Set select value
                                            const select = document.querySelector(`[name="social_links[{{ $i }}][platform]"]`);
                                            if (select) {
                                                select.value = '{{ $link['platform'] ?? '' }}';
                                                updatePlatformIcon({{ $i }}, '{{ $link['platform'] ?? '' }}');
                                            }

                                            // Set URL value
                                            const urlInput = document.querySelector(`[name="social_links[{{ $i }}][url]"]`);
                                            if (urlInput) {
                            ...
                            @else
                                <div class="text-center py-8 text-gray-500 dark:text-gray-400" id="emptyState">
                                    <svg class="w-16 h-16 mx-auto mb-3 text-gray-400 dark:text-gray-600"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                                    </svg>
                    ...
                    <div class="p-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="text" name="meta_title" placeholder="SEO title for this member"
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" rows="3"
                                ...
                                focus:border-[#1363C6] focus:ring-4 focus:ring-[#1363C6]/10 
                                dark:bg-gray-800 dark:text-white transition-all duration-300 resize-none"
                                placeholder="Brief description for search engines (150-160 characters)">{{ old('meta_description', $teamMember->meta_description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-4">
                    <a href="{{ route('team-members.index') }}"
                        class="px-5 py-2.5 bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-300 
                        rounded-xl hover:shadow-lg hover:shadow-gray-400/30 hover:-translate-y-0.5
                        transition-all duration-300 font-medium">
                        Cancel
                    </a>

                    <button type="submit"
                        class="px-5 py-2.5 bg-gradient-to-r from-[#1363C6] to-[#1e7ed8] 
