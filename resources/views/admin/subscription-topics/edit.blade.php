<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h2 class="font-bold text-gray-800 dark:text-gray-200 leading-tight">
                    {{ __('Edit Subscription Topic') }}
                </h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Update subscription topic: <span class="font-semibold">{{ $subscriptionTopic->name }}</span>
                </p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <form action="{{ route('admin.subscription-topics.update', $subscriptionTopic) }}" method="POST"
            class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="name" id="name" required
                                    value="{{ old('name', $subscriptionTopic->name) }}"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="slug" id="slug" required
                                    value="{{ old('slug', $subscriptionTopic->slug) }}"
                                    class="pl-10 w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-colors"
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            ...
                            placeholder="Describe what subscribers will receive when they subscribe to this topic...">{{ old('description', $subscriptionTopic->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1.5 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
                    ...
                    <div
                        ...
                        <div class="flex items-center h-5">
                            <input id="is_active" name="is_active" type="checkbox" value="1"
                                {{ old('is_active', $subscriptionTopic->is_active) ? 'checked' : '' }}
            ...
            <div
                ...
                <div
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Topic Statistics</h3>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                            <div class="text-3xl font-bold text-blue-700 dark:text-blue-400">
                                {{ $subscriptionTopic->subscribers()->count() }}
                            </div>
                            <div class="text-sm text-gray-600 dark:text-gray-400 mt-2">Total Subscribers</div>
                        </div>
                        <div class="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                            <div class="text-3xl font-bold text-green-700 dark:text-green-400">
                                {{ $subscriptionTopic->subscribers()->where('status', 'verified')->count() }}
                            </div>
                        ...
                        <div class="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                            <div class="text-3xl font-bold text-purple-700 dark:text-purple-400">
                                {{ $subscriptionTopic->campaigns()->count() }}
                            </div>
                            <div class="text-sm text-gray-600 dark:text-gray-400 mt-2">Campaigns</div>
                        </div>
                        <div class="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                            <div class="text-3xl font-bold text-gray-700 dark:text-gray-300">
                                {{ $subscriptionTopic->created_at->diffForHumans() }}
                            </div>
                            <div class="text-sm text-gray-600 dark:text-gray-400 mt-2">Created</div>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1" id="preview-description">
                                    {{ $subscriptionTopic->description ?: 'Topic description will appear here' }}
                                </p>
                            </div>
                        </div>
                    </div>
