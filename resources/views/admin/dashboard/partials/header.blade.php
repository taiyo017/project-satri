...
<header x-data="{ menuToggle: false }"
    class="sticky top-0 z-50 w-full border-b border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
    <div class="flex items-center justify-between px-4 py-3 lg:px-6 lg:py-4">
        <div class="flex items-center gap-3">
            <button @click.stop="sidebarToggle = !sidebarToggle"
                ...
                class="flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition-all duration-200 hover:bg-gray-100 hover:text-gray-700 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-300 lg:h-11 lg:w-11"
                aria-label="Toggle Sidebar">
                <svg class="hidden fill-current lg:block" width="20" height="20" viewBox="0 0 20 20" fill="none"
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center lg:hidden">
                <img class="h-8 dark:hidden" src="{{ $logoPath }}" alt="{{ config('app.name') }}" />
                <img class="hidden h-8 dark:block" src="{{ $logoPath }}" alt="{{ config('app.name') }}" />
            </a>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                <button @click.prevent="dropdownOpen = !dropdownOpen"
                    class="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 transition-all duration-200 hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-800 lg:px-4"
                    aria-label="User Menu">
                    <div
                        class="flex h-8 w-8 items-center justify-center rounded-full bg-[#007595] text-sm font-semibold text-white">
                        {{ strtoupper(substr($user->name, 0, 1)) }}
                    </div>

                    <span class="hidden text-sm font-medium text-gray-700 dark:text-gray-300 lg:block">
                        {{ $user->name }}
                    </span>
                ...
                <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="border-b border-gray-200 px-4 py-3 dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-full bg-[#007595] text-base font-semibold text-white">
                                {{ strtoupper(substr($user->name, 0, 1)) }}
                            </div>
                    ...
                    <div class="py-2">
                        <a href="{{ route('profile.edit') }}"
                            class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700">
