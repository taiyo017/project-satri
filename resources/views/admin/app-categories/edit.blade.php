<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 sm:mt-1">
                    Update category: {{ $appCategory->name }}
                </p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <form action="{{ route('app-categories.update', $appCategory) }}" method="POST" x-data="categoryForm()">
            @csrf
            @method('PUT')
                        ...
                        <div class="p-6 space-y-5">
                            <div>
                                ...
                                <x-text-input id="name" type="text" name="name" class="mt-2 block w-full"
                                    :value="old('name', $appCategory->name)" x-model="name" @input="generateSlug" required />
                            ...
                            <div>
                                ...
                                <div class="relative mt-2">
                                    <x-text-input id="slug" type="text" name="slug" class="block w-full pr-28"
                                        :value="old('slug', $appCategory->slug)" x-model="slug" required />
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="4"
                                    class="mt-2 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-lg shadow-sm resize-none">{{ old('description', $appCategory->description) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <x-text-input id="icon" name="icon" type="text" class="mt-2 block w-full"
                                        :value="old('icon', $appCategory->icon)" />
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2 mt-2">
                                        <input type="color" id="color" name="color" value="{{ old('color', $appCategory->color) }}"
                                            class="h-10 w-16 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer">
                                        <x-text-input type="text" class="flex-1" value="{{ old('color', $appCategory->color) }}" readonly />
                                    </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="order" name="order" type="number" class="mt-2 block w-full"
                                    :value="old('order', $appCategory->order)" min="0" />
                                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <div class="p-6 space-y-4">
                            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                                ...
                                <input type="checkbox" id="is_active" name="is_active" value="1"
                                    {{ old('is_active', $appCategory->is_active) ? 'checked' : '' }}
    ...
    @push('scripts')
        <script>
            function categoryForm() {
                return {
                    name: '{{ old('name', $appCategory->name) }}',
                    slug: '{{ old('slug', $appCategory->slug) }}',
                    generateSlug() {
                        this.slug = this.name
                            .toLowerCase()
                            .replace(/[^a-z0-9]+/g, '-')
